...
function addTestBackdropLocal(name, position, orientation, initData) {  
...
function addCasesAt(origin, orientation, testCases, initData) {
    var models = addTestBackdropLocal("Material_matrix_backdrop", origin, orientation, initData);
...
addCases = function (testCases, initData) {
    MyAvatar.orientation = Quat.fromPitchYawRollDegrees(0.0, 0.0, 0.0);
    var orientation = MyAvatar.orientation;
    orientation = Quat.safeEulerAngles(orientation);
    orientation.x = 0;
    orientation = Quat.fromVec3Degrees(orientation);
    var root = Vec3.sum(initData.originFrame, Vec3.multiply(ROOT_Z_OFFSET, Quat.getForward(orientation)));
    root = Vec3.sum(root, Vec3.multiply(ROOT_Y_OFFSET, Quat.getUp(orientation)));

    return addCasesAt(root, orientation, testCases, initData);
}
